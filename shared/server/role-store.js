...
function createRoleStore(readFromStorage, writeToStorage, options = {}) {
  ...
  async function revokeRole(email, role, actor) {
    ...
    if (RoleModel) {
      ...
      if (role === 'admin') {
        // Atomic guard: only pull admin if more than one admin exists.
        // Uses a filter that requires at least 2 admin docs to exist,
        // preventing the TOCTOU race where two concurrent revocations
        // both pass a count check and remove the last two admins.
      ...
      return { email: normalized, roles: updated ? updated.roles : [] };
    }
    ...
    if (RoleModel) {
      ...
      for (const doc of docs) {
        ...
        if (existing) {
          ...
          await RoleModel.updateOne({ email: newEmail }, { $set: { roles: mergedRoles } });
        } else {
